...
def try_wrds_pull():
    ...
    try:
        ...
        # Compustat funda (minimal set; we will enrich NI and SIC later if needed)
        ...
        for df in (comp, company, security, execu, link, bx, bx_stocks):
            if not df.empty:
                df.columns = [c.lower() for c in df.columns]

        return {"comp": comp, "company": company, "security": security, "execu": execu, "link": link, "bx": bx, "bx_stocks": bx_stocks, "sp1500": sp1500}
...
def coalesce_sources(wrds_dict):
    ...
    security = wrds_dict.get("security", pd.DataFrame())
    ...
    return comp, execu, link, bx, bx_stocks, sp1500
...
def build_and_merge(comp, execu, link, bx, bx_stocks, security):
    ...
    if not execu.empty:
        ...
        # female_ceo from gender or ceo_gender
        ...
        elif 'ceoann' in execu.columns:
            execu['female_ceo'] = pd.NA  # cannot infer gender from ceoann
        # becameceo_year
...
def construct_vars(df):
    ...
    df['ln_assets'] = np.log(pd.to_numeric(df.get('at'), errors='coerce'))
...
def apply_filters(df, sp1500):
    ...
    if not sp1500.empty:
        ...
        if 'from' in sp.columns and 'thru' in sp.columns:
            ...
            # Create year ranges for each gvkey
            sp_list = []
            for _, row in sp.iterrows():
                if pd.notna(row['from']) and pd.notna(row['thru']):
        ...
        # Handle S&P 500 date range format from WRDS (fallback)
            ...
            sp_list = []
            for _, row in sp.iterrows():
                if pd.notna(row['from_date']) and pd.notna(row['thru_date']):
        ...
        if not sp.empty:
            ...
            df = df.merge(sp[keep_cols], on=['gvkey','fyear'], how='left')
            print(f"DEBUG: After S&P 1500 filter: {len(df)} observations")
...
def main():
    ...
    comp, execu, link, bx, bx_stocks, sp1500 = coalesce_sources(wrds_dict)
    print("DEBUG: Data sources coalesced successfully")

    # 2) Merge & construct variables
    df = build_and_merge(comp, execu, link, bx, bx_stocks, wrds_dict.get("security", pd.DataFrame()))
